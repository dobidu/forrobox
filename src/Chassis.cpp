...
void Chassis::pollVisualisers()
{
    ...
    if (timbreValue != nullptr)
        setTimbreIndex (juce::roundToInt (timbreValue->load (std::memory_order_relaxed)));
...
float Chassis::swayDegreesAt (double phaseSeconds) noexcept
{
    ...
    return keyframeValueAt (phaseSeconds, drunk::kSwaySeconds,
                            { { 0.00,  0.0f },
...
void Chassis::setTimbreIndex (int choiceIndex)
{
    // By the TABLE, never by a literal 2 and never by a literal "ciclo" either.
    // `ids::channelInfos`' own header states the law: a cast from an index
    // straight to a meaning is how a strip paints in another instrument's
    // colour, and a test that restates the rule cannot see a divergence in it.
    const auto ciclotron = choiceIndex == ciclotronTimbreIndex();

    // AND NOTHING ELSE. `KeyframeLoop` owns its own `PollTimer` and starts it
    // in `setRunning` — that is the whole of what 08-04 built it for. This
    // function first drove the flicker from a SECOND poll beside it, so every
    // frame advanced the phase twice and the 4 s cycle ran in 2 s.
    //
    // Nothing in the suite could see it, and that is a property of how these
    // animations are tested rather than an oversight: each is TOLD its elapsed
    // time and every check drives it by hand with no message loop running,
    // which is what makes them reliable (04-04, three checks lost to MSVC's
    // clock) and is exactly why two drivers calling one `advance` are
    // indistinguishable. What IS observable is how many clocks exist —
    // `PollTimer::runningCount` and `testTurningOnAnEffectAddsOneClock`.
    effectOverlay.setCiclotron (ciclotron);
}

void Chassis::setCachacaPercent (float percent)
{
    effectOverlay.setAmount (EffectOverlay::amountFor (percent));

    const auto nowTipsy = percent >= drunk::kTipsyPercent;
...
void Chassis::attachParameters (juce::AudioProcessorValueTreeState& apvts, ValueTooltip* tooltip)
{
    ...
    timbreValue  = apvts.getRawParameterValue (ids::timbre);
...
void Chassis::resized()
{
    ...
    effectOverlay.setBounds (getLocalBounds());
