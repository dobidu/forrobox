...
| ~~Stubbed controls: `LOAD`, `LOAD IR…`, preset cycler, `PAT 01–08`, `MULTI-OUT`~~ | Init | — | **ALL RESOLVED IN PHASE 9**, inside v0.1 rather than after it: `MULTI-OUT` at 04-06, `PAT 01–08` at 09-05, the preset cycler at 09-06, `LOAD IR…` at 09-07 and per-strip `LOAD` at 09-08. The README's feature table no longer carries a stub row |
...
| ~~Extract `PROFILES` from `data.js` into a `profiles.json` consumed by both the prototype and the cross-check~~ | 2 | — | **RESOLVED AT 09-01.** The boundary decision it was blocked on was taken with the user: `data.js` is PARTLY generated — its `PROFILES` and `PROFILE_ORDER` blocks only — and everything else in it stays hand-written and authoritative. `scripts/build-profiles.py` renders both sides from `assets/profiles.json` and `--verify` is a gate. Original note follows. |
| (the entry above, as it was written at Phase 2) | 2 | M | The root fix for parsing `data.js` with regexes, raised by `/simplify`. Blocked on a boundary decision: it modifies `data.js` and the prototype, both read-only. Revisit if the extractor breaks again |
| ~~`ForroBoxTests.exe` prints its result under MSVC and never exits~~ | 07-03 | — | **RESOLVED at 08-02, and the cause was not the test binary.** Four occurrences across 07-03, 08-01 and 08-02, each misread as the suite hanging and each 'fixed' by killing the PID and re-running a full MSVC cycle. The exe is fine: measured three ways on the same binary, `exe > file` exits 0, `exe > file` then `cat` exits 0, and `exe 2>&1 | tee file` NEVER exits (timed out at 240 s). Piping a WINDOWS process's output into a Linux reader under WSL interop hangs after the final write, and `scripts/build-windows.sh`'s `run()` piped every command through `tee` — so the script never reached its install, hash and moduleinfo steps. `run()` now captures to a temp file and appends afterwards. The hypothesis recorded here an hour earlier — a non-joined JUCE `TimerThread` — was WRONG, and is kept in this sentence rather than deleted: it was plausible, it was written down as unverified, and the thing that settled it was running the binary three ways rather than reasoning about threads. **AND THE CONCLUSION DRAWN HERE WAS STILL TOO STRONG.** Removing the pipe was a real fix, but this entry reads as though it restored the install path — and it did not. The binary still never exits, so `run()` still never returned and the script still never reached its install step: `--install` remained UNREACHABLE until 2026-09-24 (`b55135e`), which is the day someone first tried to use it. Killing the binary by hand instead makes `run()` fail and the script exit FATAL, which is the same dead end from the other side. What actually restored it was bounding the wait and judging the suite by the summary line it prints. Two bugs wore one symptom, and fixing the first was read as fixing both |
...
| `Chassis` gained a fifth concern; the settings menu wants extracting | 08-02 | M | 04-04's `/simplify` said `Chassis` must not grow a third region without splitting first. The header/footer/side-panel split happened, and 08-02 added the settings menu whole: `Chassis.cpp` is now the largest file in `src/` and `Chassis.h` gained six public members for one feature. The named fix is a `SettingsMenu` unit owning the id enum, `kAccentSteps`, `build()` and `apply(int) -> Result` — it needs `Settings&` and nothing else. **The cost is already visible**: the id enum is in an anonymous namespace, so `tests/UiTest.cpp` hard-codes `101`, `300 + 0`, `400 + 9`, `100 + 7` at ~24 sites — the id table transcribed a second time, in literals, because the menu has nowhere of its own to live. Raised by `/simplify`'s altitude and simplification passes |
